...
vector<vector<string>> blocked_output;

bool isblocked(string a, string b = ".", string c = "."){
    if(blocked.find(a) != blocked.end()) return true;
    if(blocked.find(b) != blocked.end()) return true;
    if(blocked.find(c) != blocked.end()) return true;
    return false;
}
...
int load_word_from_memory(int address)
{
    int n = 0;
    for (int i = 0; i < 4; i++)
    {
...
void store_word_to_memory(int n, int address)
{
    for (int i = 0; i < 4; i++)
    {
        memory[(address + i)/1024][(address + i)%1024] = ((n >> (24 - 8 * i)) & 0xFF);
    }
}
...
int process(vector<string> &instruction, int num)
{
    ...
    if (instruction[0] == "add")
    {
        ...
        if(loading == true and isblocked(a, b, c)){
            blocked.insert(a);
            output_size += temp_output.size();
            blocked_output.push_back({"cycle "+to_string(cycle), getstr(instructions[num]), a+" = "+to_string(registers[a]), "-", "-"});
        }
    ...
    else if (instruction[0] == "addi")
    {
        ...
        if(loading == true and isblocked(a, b)){
            blocked.insert(a);
            output_size += temp_output.size();
            blocked_output.push_back({"cycle "+to_string(cycle), getstr(instructions[num]), a+" = "+to_string(registers[a]), "-", "-"});
        }
    ...
    else if (instruction[0] == "lw")
    {
        ...
        else
        {
            if(loading == true or storing == true){
                ...
                int counter = 0;
                for(auto u:blocked_output){
                    output.push_back({"cycle "+to_string(output.size()+1+(counter++)), u[1], u[2], u[3], u[4]});
                }
                cycle = output.size()+1;
                temp_output.clear();
                blocked_output.clear();
                loading_cycle = 0;
            }
            storing = false;
            loading = true;
            registers[a] = load_word_from_memory(memory_access_index);
            lr = a;
            blocked.insert(a);
            output_size++;
            ...
            if(buffer_ind == -1){
                ...
                for(int i=0;i<row_delay;i++){
                    temp_output.push_back({"cycle "+to_string(i+cycle+row_delay+1), "DRAM: Writeback row "+to_string(buffer_ind), "-", "-", "Activate row "+to_string(row)});
                }
                for(int i=0;i<col_delay;i++){
    ...
    else if (instruction[0] == "sw")
    {
        ...
        else
        {
            ...
            storing = true;
            loading = false;
            store_word_to_memory(registers[a], memory_access_index);
            lr = a;
            blocked.insert(a);
            output_size++;
            output.push_back({"cycle "+to_string(cycle), getstr(instructions[num]), "-", "-", "DRAM request issued"});
...
int main()
{
    ...
    for(int i=0;i<1024*1024;i += 4){
        if(load_word_from_memory(i) != 0){
            cout << i << "-" << i+3 << ": " << load_word_from_memory(i) << endl;
        }
    }
    ...
    compress();
    print(output);
    cout << "Final Register Values: " << endl << endl;
    printRegisters();
    cout << endl << endl;
}